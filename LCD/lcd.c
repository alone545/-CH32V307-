...
#define X_START_OFFSET  0
#define Y_START_OFFSET  17
...
void LCD_Init(void)
{
    ...
    // 修正初始化序列中的X/Y范围（0x2A/0x2B命令）
    LCD_WR_REG(0x2A); // X地址设置
    LCD_WR_DATA(0x00); // X起始高8位
    LCD_WR_DATA(0x00); // X起始低8位
    LCD_WR_DATA(0x01); // X结束高8位 (279=0x0117)
    LCD_WR_DATA(0x17); // X结束低8位

//    LCD_WR_REG(0x2B); // Y地址设置
//    LCD_WR_DATA(0x00); // Y起始高8位
//    LCD_WR_DATA(0x00); // Y起始低8位
//    LCD_WR_DATA(0x00); // Y结束高8位 (239=0x00EF)
//    LCD_WR_DATA(0xEF); // Y结束低8位
    // 修改Y设置
    LCD_WR_REG(0x2B);
    LCD_WR_DATA(0x00); // Y start high
    LCD_WR_DATA(0x00); // Y start low
    LCD_WR_DATA(0x01); // Y end high (0x013F = 319)
    LCD_WR_DATA(0x3F); // Y end low
//    LCD_WR_REG(0x2B);
//    LCD_WR_DATA(0x00);
//    LCD_WR_DATA(0x00);
//    LCD_WR_DATA(0x01);
...
void LCD_drawImageWithSize(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint32_t addr)
{
    ...
    while (DMA_GetFlagStatus(SPI2_DMA_TC_FLAG) == RESET);
